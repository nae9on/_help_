...
#define MXTHREADS 100

int main(int argc, char** argv)
{
  int size=2000,tilesize=200,xtiles,ytiles,xc,yc,count,tilebase,i,me;
  char *tile[MXTHREADS], *picture;
  double ws,we,cs,ce;
...
#pragma omp parallel 
#pragma omp single
  if (omp_get_num_threads() > MXTHREADS) {
      fprintf(stderr,"MXTHREADS is smaller than number of threads!\n");
      exit(1);
  }
  
#pragma omp parallel private(xc,yc,i,me) \
            shared(picture,tile,tilebase,tilesize,xtiles,ytiles,size,stderr) \
            default(none)
  {
    me = omp_get_thread_num();
    if((tile[me]=(char*)malloc(tilesize*tilesize*sizeof(char)))==NULL)
      {
...
#pragma omp single 
    ...
    for(yc=0; yc<xtiles; yc++)
      ...
      {
#pragma omp task private(tilebase,i,me) firstprivate(xc,yc) \
            shared(tile,picture,size,tilesize,xtiles) default(none)
        {
        me = omp_get_thread_num();
	/* calc one tile */
	calc_tile(size, xc*tilesize, yc*tilesize, tilesize, tile[me]);
	/* copy to picture buffer */
	for(i=0; i<tilesize; i++)
	  {
	    tilebase=yc*tilesize*tilesize*xtiles+xc*tilesize;
	    memcpy((void*)(picture+tilebase+i*tilesize*xtiles),
		   (void*)(tile[me]+i*tilesize),
