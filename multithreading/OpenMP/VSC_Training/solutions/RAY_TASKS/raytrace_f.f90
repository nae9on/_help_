...
program raytracef
  ...
  use mod_raytracef
  implicit none
  integer, parameter :: size = 2000, tilesize = 200, mxthr = 100
  integer :: xtiles, ytiles, xc, yc, tilebase, ix, iy, me
  type :: c
    integer(kind=is), allocatable :: tile(:,:)
  end type
  type(c) :: set(mxthr)
...
!$omp parallel
!$omp single
  if (omp_get_num_threads() > mxthr) stop 'mxthr is smaller than thread count'
!$omp end single
!$omp end parallel

!$omp parallel private(xc,yc,me) &
!$omp          shared(picture,set,tilebase,xtiles,ytiles) &
!$omp          default(none)
  me = omp_get_thread_num()
  allocate(set(me)%tile(tilesize, tilesize))
!$omp barrier
!$omp single
  do yc = 0, xtiles - 1
     do xc = 0, xtiles - 1
!$omp task private(tilebase,me) firstprivate(xc,yc) &
!$omp      shared(set,picture,xtiles) default(none)
        me = omp_get_thread_num()
        call calc_tile(size, xc*tilesize, yc*tilesize, tilesize, set(me)%tile)
        picture(xc*tilesize+1:(xc+1)*tilesize, yc*tilesize+1:(yc+1)*tilesize) = set(me)%tile
!$omp end task
     end do
  end do
!$omp end single
  deallocate(set(me)%tile)
!$omp end parallel
