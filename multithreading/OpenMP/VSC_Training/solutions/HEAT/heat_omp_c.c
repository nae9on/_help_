...
int main()
{
  double eps = 1.0e-08;
  double **phi, **phin;
  double dx,dy,dx2,dy2,dx2i,dy2i,dt,dphi,dphimax,dphimax_glob;
...
/* iteration */
#pragma omp parallel private(i,k,it,dphi,dphimax) 
...
#pragma omp single nowait
    ...
    {
#pragma omp for reduction(max:dphimax_glob)
	for (i=1;i<imax;i++)
	{
	    for (k=1;k<kmax;k++)
	    {
...
// Barrier in next omp for protects against subsequent update of dphimax_glob
// save values 
#pragma omp for
	for (i=1;i<imax;i++)
	{
...
#pragma omp single nowait
  ...
  printf("\nCPU time = %#12.4g sec\n",ti);
  mflops = 11.0 * (float) (kmax-1)*(float) (imax-1)*(float)it_glob / 1.e6;
  printf("\nMFlop/s = %#12.4g \n",mflops/ti);
